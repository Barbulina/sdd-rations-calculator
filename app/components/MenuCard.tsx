...
export function MenuCard({ menu, onDelete }: MenuCardProps) {
  ...
  return (
    <div
      ...
      className="relative bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700 mb-3"
    >
      {/* Entire card is the link */}
      <Link
        href={`/menu/${menu.id}`}
        className="block p-4 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition rounded-lg"
      >
        <div className="pr-10">
        ...
        <div className="flex gap-4 mt-3 pt-3 border-t border-gray-100 dark:border-gray-700">
          ...
          <div className="text-center">
            ...
            <p className="text-sm font-semibold text-gray-800 dark:text-gray-200">
              {Math.round(menu.totalWeight)}g
            </p>
          </div>
        </div>
      </Link>

      {/* Delete button — absolutely positioned to float above the link */}
      <button
        onClick={(e) => {
          e.stopPropagation();
          onDelete(menu.id);
        ...
        className="absolute top-3 right-3 p-2 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded transition"
      >
        <svg
