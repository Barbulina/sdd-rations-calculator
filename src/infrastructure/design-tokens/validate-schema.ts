...
function validateColorToken(
  ...
  if (typeof t.value !== "string" || !/^#[0-9A-Fa-f]{6}$/.test(t.value)) {
    throw new Error(
      `Invalid color token ${name}: value must be a 6-digit hex code`,
    );
  }
  ...
  if (t.theme !== "light" && t.theme !== "dark") {
    throw new Error(
      `Invalid color token ${name}: theme must be 'light' or 'dark'`,
    );
  }
  ...
  if (t.$extensions) {
    if (!t.$extensions.m3Role || !t.$extensions.m3Tone) {
      throw new Error(
        `Invalid color token ${name}: $extensions must include m3Role and m3Tone`,
      );
    }
  }
}
...
function validateTypographyToken(
  token: unknown,
  name: string,
): asserts token is TypographyToken {
  if (!token || typeof token !== "object") {
  ...
  if (t.type !== "typography") {
    throw new Error(
      `Invalid typography token ${name}: type must be 'typography'`,
    );
  }

  if (!t.value || typeof t.value !== "object") {
    throw new Error(
      `Invalid typography token ${name}: value must be an object`,
    );
  }
  ...
  for (const prop of requiredProps) {
    if (!(prop in t.value)) {
      throw new Error(
        `Invalid typography token ${name}: value missing required property ${prop}`,
      );
    }
  }
}

function validateSpacingToken(
  token: unknown,
  name: string,
): asserts token is SpacingToken {
  if (!token || typeof token !== "object") {
  ...
  if (typeof t.value !== "string" || !/^\d+(\.\d+)?rem$/.test(t.value)) {
    throw new Error(
      `Invalid spacing token ${name}: value must be in rem units`,
    );
  }
  ...
  if (t.$extensions) {
    if (
      typeof t.$extensions.pixelValue !== "number" ||
      typeof t.$extensions.gridFactor !== "number"
    ) {
      throw new Error(
        `Invalid spacing token ${name}: $extensions must include pixelValue and gridFactor`,
      );
    }
    ...
    if (name !== "space-0" && t.$extensions.pixelValue % 8 !== 0) {
      throw new Error(
        `Invalid spacing token ${name}: pixelValue must be a multiple of 8 (got ${t.$extensions.pixelValue})`,
      );
    }
  }
}
