...
async function main() {
  ...
  try {
    ...
    console.error('❌ Failed to parse tokens.json:', error instanceof Error ? error.message : String(error));
    process.exit(1);
  }
  ...
  for (const [name, token] of Object.entries(tokens['category-colors'])) {
    ...
    if (!result.passes) {
      ...
      console.log(`  ✓ ${name} (${token.theme}): ${result.ratio}:1 (${result.level})`);
    }
  }
  ...
  try {
    ...
    console.error('❌ Style Dictionary transformation failed:', error instanceof Error ? error.message : String(error));
    process.exit(1);
  }
