...
import { MenuItemBuilder, createTestAliment } from './MenuItemBuilder';
...
describe('MenuItem', () => {
  describe('creation', () => {
    it('should create MenuItem with valid data', () => {
      ...
      expect(menuItem.id).toMatch(/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i);
  ...
  describe('weight validation', () => {
    it('should reject zero weight', () => {
      ...
      expect(() => new MenuItem(aliment, 0)).toThrow('Weight must be greater than 0');
    });

    it('should reject negative weight', () => {
      const aliment = createTestAliment();
      
      expect(() => new MenuItem(aliment, -10)).toThrow('Weight must be greater than 0');
    });

    it('should reject weight exceeding 10kg (10000g)', () => {
      const aliment = createTestAliment();
      
      expect(() => new MenuItem(aliment, 10001)).toThrow('Weight cannot exceed 10000g');
    });
