...
describe('MenuItemsList', () => {
  ...
  describe('aliment data display', () => {
    ...
    it('should display glycemic index when available', () => {
      ...
      const elements = screen.getAllByText((content, element) => {
        return element?.textContent?.includes('GI') && element?.textContent?.includes('38');
      });
    ...
    it('should not display custom badge for catalog aliments', () => {
      ...
      expect(screen.queryByText(/custom|personalizado/i)).not.toBeInTheDocument();
    });
  });
  ...
  describe('weight inline editing', () => {
    ...
    it('should display current weight value', () => {
      ...
      const weightInput = screen.getByLabelText(/weight|peso/i) as HTMLInputElement;
  ...
  describe('rations display', () => {
    ...
    it('should update rations display when weight changes', () => {
      ...
      const updatedItems = [{ ...mockItems[0], weightGrams: 300, rations: 2.0 }];
      rerender(
        <MenuItemsList
          items={updatedItems}
          onUpdateWeight={onUpdateWeight}
  ...
  describe('remove button', () => {
    it('should render remove button for each item', () => {
      ...
      const removeButtons = screen.getAllByRole('button', { name: /remove|eliminar|delete/i });
    ...
    it('should call onRemoveItem when remove button clicked', () => {
      ...
      const removeButton = screen.getByRole('button', { name: /remove|eliminar|delete/i });
  ...
  describe('empty state', () => {
    it('should show empty state when no items', () => {
      ...
      expect(screen.getByText(/no aliments added|no hay alimentos|añade alimentos/i)).toBeInTheDocument();
    });
    ...
    it('should not show empty state when items exist', () => {
      ...
      expect(screen.queryByText(/no aliments added|no hay alimentos/i)).not.toBeInTheDocument();
    });
  ...
  describe('styling and layout', () => {
    it('should render items in a list container', () => {
      ...
      const container = screen.getByTestId('menu-items-list') || screen.getByRole('list');
